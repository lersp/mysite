document.addEventListener('DOMContentLoaded', () => {
    ...
    .then(([config, data]) => {
        ...
        hobbiesList.innerHTML = perfil.hobbies.map(hobby => `<li>${hobby}</li>`).join('');

        const techList = document.getElementById('lista-tech');
        const allSkills = [...perfil.competencias.especialidades, ...perfil.competencias.tecnicas];
        const uniqueSkills = [...new Set(allSkills)]; // Remove duplicatas
        techList.innerHTML = uniqueSkills.map(skill => `<li>${skill}</li>`).join('');

        const studyList = document.getElementById('lista-study');
        const formacaoHtml = perfil.formacao.map(f => `<li><b>${f.curso}</b> - ${f.instituicao} (${f.periodo || 'Previsão: ' + f.previsao_conclusao})</li>`).join('');
        const certsHtml = perfil.certificacoes.map(c => `<li>${c}</li>`).join('');
        studyList.innerHTML = formacaoHtml + certsHtml;

        const idiomList = document.getElementById('lista-idiom');
        idiomList.innerHTML = perfil.idiomas.map(i => `<li><b>${i.idioma}:</b> ${i.nivel}</li>`).join('');
